...
- **AWS Section Review is topic-based + nested** (our study-module taxonomy in `content/aws-saa/topics.js`): review a whole domain, a study module, or a single sub-topic. Each review target has its own Latest/Best/#taken. Overall Test still uses the 4 exam domains for weighting. Attempts carry a `reviewId` (domain key, `sec:<id>`, or `sub:<id>`).
...
- **Phase 5 — DSA expansion:** 6 more array/string/bit problems → 10 total across 7 topics. Per-test `hidden` flag (redacted as "Hidden test" in UI, still graded). Solve time shown on run; DsaHome cards show best solve time + attempt count.
...
- **DSA expansion (10 → 25 problems):** added DP, math, arrays, binary search, two pointers, sliding window, and graph exercises with an easy→medium→hard ramp; manually verified expected values incl. hidden tests. `DsaHome.jsx` gained a difficulty filter + difficulty-sorted display + color-coded `.diff` tags.
- **DSA design problems (25 → 30) + engine support:** added `kind: "design"` path to `worker.js`/`runner.js` (instantiate class, replay op sequence, compare outputs); shows "Define a class named X". Added stack/queue, key-value store, trie, and bounded-cache design exercises. Verified via reference solutions. Tab relabeled "DSA".
- **Real World track (3rd tab):** generalized code engine into `tracks/CodeTrack.jsx` + `components/code/CodeHome.jsx` + `CodeSolve.jsx` (props: trackKey/title/intro/problems/topics); App renders AwsTrack + two CodeTracks; removed old `components/dsa/*` and `tracks/DsaTrack.jsx`. New `content/realworld/problems.js` with 13 practical exercises (calendar/schedule helpers, parsing, pagination, aggregation, dedupe, run compression, IPv4, backoff, rate limiting, cart, scoreboard). Verified via reference solutions.
